...
$pdf->Cell(0,0.7,'Laporan Data Penyaluran Zakat Fitrah Uang',0,0,'C');
...
$data = mysqli_query($koneksi,"SELECT * FROM donasi JOIN zakatfitrah ON donasi.id_donasi = zakatfitrah.id_donasi WHERE tgl_donasi BETWEEN '$tglawal' AND '$tglakhir' AND jenis_donasi = 'zakat_uang' AND status IN (2) ORDER BY id_zakat,tgl_donasi ASC");
while ($row=$data->fetch_array()) {
    $pdf->ln();
    $pdf->SetFont('Arial','B',9);

    $pdf->Cell(1, 0.8, $no.'.', 1, 0, 'C');
    $pdf->Cell(4, 0.8, date('d-m-Y', strtotime($row['tgl_donasi'])), 1, 0, 'C');
    $pdf->Cell(4, 0.8, $row['nama_donatur'], 1, 0, 'C');
    $pdf->Cell(7, 0.8, $row['alamat'], 1, 0, 'C');
    $pdf->Cell(3.5, 0.8, $row['nilai_zakat'], 1, 0, 'C');
    $pdf->Cell(4, 0.8, $row['jumlah_tanggungan'], 1, 0, 'C');
    $pdf->Cell(3.5, 0.8, "Rp. ".number_format($row['total']).",-", 1, 0, 'C');
...
while($tampil = $data->fetch_array()){
    ...
    $pdf->Cell(3.5, 0.8, "Rp. ".number_format($jumlah).",-", 0, 0, 'C');
