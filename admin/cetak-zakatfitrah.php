...
$data = mysqli_query($koneksi,"SELECT * FROM donasi,zakatfitrah WHERE donasi.id_donasi = zakatfitrah.id_donasi AND jenis_donasi = 'zakat_uang' AND date(tgl_donasi)>='$tgl_dari' AND date(tgl_donasi)<='$tgl_sampai' AND status IN (2) ORDER BY id_zakat,tgl_donasi ASC");
